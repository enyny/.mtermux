...
termux-setup-storage && termux-wake-lock && pkg update -y && pkg i -y git && mkdir -p $HOME/bin $HOME/git $HOME/.termux && cd git && git clone --depth 1 https://github.com/enymy/.mtermux.git && ln -sf $HOME/git/.mtermux/my-termux-url-opener $HOME/bin/termux-url-opener && ln -sf $HOME/git/.mtermux/my-termux-url-opener $HOME/bin/termux-file-editor
